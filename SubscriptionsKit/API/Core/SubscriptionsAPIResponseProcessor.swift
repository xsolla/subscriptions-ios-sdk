...
{
    typealias SubscriptionsAPIError = APIError<SubscriptionsAPIErrorModel>
...
extension SubscriptionsAPIResponseProcessor: ResponseProcessor
{
    ...
    {
        logger.debug(.networking, domain: .subscriptionsKit)
        {
        ...
        if let error: SubscriptionsAPIError = errorHandler.error(request: request,
        ...
        // Null data error
        if data == nil { return .failure(SubscriptionsAPIError.emptyData) }
        ...
        catch let error
        {
            ...
            logger.error(.networking, domain: .subscriptionsKit)
            {
                guard let data = data,
            ...
            return .failure(SubscriptionsAPIError.decoding(error))
        }
    }
}
